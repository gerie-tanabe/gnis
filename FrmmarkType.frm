...
If IfDuplicateMT(Trim(Me.txtMarkType)) Then
    MsgBox "Mark type already exist in the list.", vbInformation, "Mark Type"
    Exit Sub
End If

    Dim rst As New ADODB.Recordset
    rst.Open "select max(mTCode) from Marktype", cnn, adOpenStatic
    cnn.Execute "insert into marktype (mtcode,mtdesc) values(" & IIf(IsNull(rst(0)), 0, rst(0)) + 1 & ",'" & Trim(StrConv(Replace(Me.txtMarkType, "'", "''"), vbProperCase)) & "')"
