...
Begin VB.Form FrmMarkStatus 
   ...
   Begin Rave_Buttons.RaveButtons cmdOK 
      ...
      MCOL            =   16777215
      MPTR            =   1
      MICON           =   "FrmMarkStatus.frx":0000
      PICN            =   "FrmMarkStatus.frx":001C
      UMCOL           =   -1  'True
      SOFT            =   -1  'True
      PICPOS          =   0
      NGREY           =   0   'False
   ...
   End
End
Attribute VB_Name = "FrmMarkStatus"
...
If IfDuplicateMS(Trim(Me.TxtMarkStatus)) Then
    MsgBox "Mark Status already exist in the list.", vbInformation, "Mark Status"
    Exit Sub
End If

    Dim rst As New ADODB.Recordset
    rst.Open "select max(mSCode) from MarkStatus", cnn, adOpenStatic
    cnn.Execute "insert into markstatus (mscode,msdesc) values(" & IIf(IsNull(rst(0)), 0, rst(0)) + 1 & ",'" & Trim(StrConv(Replace(Me.TxtMarkStatus, "'", "''"), vbProperCase)) & "')"
